import express from "express"; // 引入 Web 框架
import { createRequire } from "module"; // 1. 引入建立 require 的功能
import "dotenv/config"; //讀取專案根目錄下的 .env 檔案，並將其中的設定內容載入到 Node.js 的 process.env 物件中。
...
router.post("/return", (req, res) => {
  ...
  try {
    //這行是啟動綠界的 SDK。options 裡面通常包含了你的 MerchantID、HashKey 和 HashIV。
    const create = new ecpay_aio_nodejs(options);
    //接下來我們要對資料進行「刪除」動作，使用展開運算子可以確保原始的 data 變數保持完整，不會被更動。
    const returnData = { ...data };
    //CheckMacValue是綠界根據它那邊的資料算出來的「檢查碼」。把它存進 receivedMac，當作標準答案。
    const receivedMac = returnData.CheckMacValue;
    //根據綠界的演算法規則：計算檢查碼時，資料物件本身不能包含 CheckMacValue 欄位。
    // 如果不刪除，你算出來的結果會把「答案」也算進去，導致計算出的結果永遠跟對方對不起來。
    delete returnData.CheckMacValue;
    //這行是在執行「我方驗算」。SDK 會抓取剛才 options 裡的 HashKey 與 HashIV，
    // 並依照綠界的規則（字母排序、串接字串、雜湊運算）對剩餘的 returnData 進行運算。
    const calculatedMac =
      create.payment_client.helper.gen_chk_mac_value(returnData);
    // 最後，將「綠界給的答案」(receivedMac) 與「我們自己算的結果」(calculatedMac) 做比對。
    // 如果相同 (true)：代表這筆資料 100% 來自綠界，且內容沒有在傳輸過程中被任何人改過。
    // 如果不同 (false)：代表資料可能被竄改過，或者是你的 HashKey/HashIV 設定錯誤。絕對不能信任這筆交易！
